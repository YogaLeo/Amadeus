...
type Location struct {
	Lat float64 `json:"lat"` //row string 首字母大写才会在location中可以访问到
	Lon float64 `json:"lon"` // 数据结构被转化为json的时候会使用row string中的内容不会被string编码改变
	// （甚至可包含space enter之类的东西）
}
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	user := r.Context().Value("user") //actually this part get token from request, not user. just name it as user
...
func readFromES(lat, lon float64, ran string) ([]Post, error) {
	...
	query := elastic.NewGeoDistanceQuery("location") // kd tree 查找
	query = query.Distance(ran).Lat(lat).Lon(lon)

	searchResult, err := client.Search().
		Index(POST_INDEX). //db
		Query(query).
		Pretty(true). // 输出结果是否美观
		Do(context.Background()) //真正实现查询
	if err != nil {
		return nil, err
	}
	...
	var ptyp Post
	var posts []Post
	for _, item := range searchResult.Each(reflect.TypeOf(ptyp)) { // 遍历 确认能否将取得的结果转化成post类型
...
func saveToBigTable(p *Post, id string) {
	ctx := context.Background()
	bt_client, err := bigtable.NewClient(ctx, PROJECT, BIGTABLE_INSTANCE, option.WithCredentialsFile(CREDENTIALS))
	if err != nil {
		panic(err)
		return
	}
...
func main() {
	fmt.Println("started-service with User, ML")
