...
var mySigningKey = []byte("secret") // 随机创建的 用于作为secret key

func checkUser(username, password string) error { // login
	...
	// 两层循环
	// 第一层是 searchResult.Each(reflect.TypeOf(utyp)) 找到所有可以被cast成user type的数据
	// 第二层是 for 和 range 组成的循环 用于 if 判断
	var utyp User
...
func addUser(user User) error { // register
	...
	if searchResult.TotalHits() > 0 { // 如果大于0代表已经有过结果了 用户存在
...
func handlerLogin(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "text/plain") // 返回数据类型
